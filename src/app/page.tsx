...
export default function Home() {
  ...
  return (
    <main className="mx-auto flex max-w-7xl flex-col items-center gap-12 px-4 py-8">
      ...
      {/* Session Status */}
      <div className="w-full max-w-2xl rounded-lg border border-neutral-200 bg-neutral-50 p-6">
        <h2 className="font-semibold">🔐 Session Status</h2>
        {isPending ? (
          <p className="text-muted-foreground mt-2 text-sm">Loading session...</p>
        ) : session ? (
          <div className="mt-3 space-y-2 rounded bg-green-50 p-3">
          ...
          <div className="mt-3 rounded bg-yellow-50 p-3">
            <p className="text-sm text-yellow-900">⚠️ No session - User not authenticated</p>
          </div>
        )}
      </div>
    </main>
